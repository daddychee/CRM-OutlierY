"""STEP 11 — SYNTHESIS: turn the pooled niche signals into a clean list of candidate BETS.
Usage: python3 11_synthesize_bets.py [workdir]   Reads videos.json, analysis.json, gaps.json -> bets.json
...
Each bet carries 4 critique signals so the Auditor argues from numbers, not vibes:
  corroboration  — how many INDEPENDENT streams back it. The lift-seed itself counts as ONE stream
                   (lift is computed FROM the outliers, so "outlier"+"lift" are not two independent
                   confirmations — audit V6); the comment-gap match adds a genuinely independent 2nd.
...
from _common import compute_outliers, jload, winners, get_scan_time, BASE_STOP
...
# Includes Vietnamese connector/filler words so VN niches don't create false-positive bets
# on generic terms like "cach", "tai", "cho", "voi", "nhung" etc.
GENERIC = set(("thing things really actually truly simply just now then here there about into onto "
               ...
               "the of to and or but with from by as is are was were be it its their "
               # Vietnamese connectors / fillers
               "cach tai cho voi nhung cai nay kia do day nao cung van dang da se rat luon ma "
               "nhung hoac la co duoc tu moi nhu khi roi nua the vay nhat hon qua lam that sao").split())

from _common import tokenize as toks       # shared stop-list (V5)

# ---------------- evidence base: valid primary outliers + early-confirmed fresh (V7) ----------------
outs = winners(v)
for o in outs:
    try:
...
def _bigrams(tokens):
    """Extract bigrams from a token list — bigrams capture word order and topical context
    better than unigrams for the coherence proxy."""
    return set(f"{tokens[i]} {tokens[i+1]}" for i in range(len(tokens)-1)) if len(tokens) > 1 else set(tokens)

bets = []
for term, lift, kind in seeds:
    # word-boundary match, not substring: seed "space" must NOT capture "spacex"/"spaces".
    pat = re.compile(r"\b" + re.escape(term) + r"\b")
    cl = [o for o in outs if pat.search(o["_tl"])]
    ...
    # COHERENCE: blend unigram Jaccard (structural) with bigram overlap (topical).
    # Pure unigram Jaccard gives high scores for titles sharing filler words ("the truth about X"
    # vs "the truth about Y" = 0.6 despite different topics). Bigrams catch "truth about" as a
    # phrase, so two titles sharing the same bigram set are genuinely about the same thing.
    tsets_uni = [[w for w in toks(o.get("title")) if w != term and w not in term.split()] for o in top]
    tsets_big = [_bigrams(ts) for ts in tsets_uni]
    pj_uni = [jaccard(tsets_uni[i], tsets_uni[j]) for i in range(len(tsets_uni)) for j in range(i + 1, len(tsets_uni))]
    pj_big = [jaccard(tsets_big[i], tsets_big[j]) for i in range(len(tsets_big)) for j in range(i + 1, len(tsets_big))]
    mean_uni = statistics.mean(pj_uni) if pj_uni else 0.0
    mean_big = statistics.mean(pj_big) if pj_big else 0.0
    coherence = round(0.4 * mean_uni + 0.6 * mean_big, 2)  # weight bigrams more: topical > structural
    ages = [o["_age"] for o in cl if o["_age"] is not None]
    med_age = int(statistics.median(ages)) if ages else None
    in_gap = any(re.search(r"\b" + re.escape(w) + r"\b", gap_text) for w in term.split())
    # lift-seed = ONE stream (lift is derived from the outliers, not independent of them — V6);
    # a comment-gap match is the only genuinely independent second stream here.
    corro = 1 + (1 if in_gap else 0)
    ...
    bets.append({
        ...
        "corroboration": corro, "streams": "+".join(
            ["lift(outlier)"] + (["gap"] if in_gap else [])),
