...
niche-research KHÔNG dính vì scripts/llm_provider.py gọi bằng `requests`
(UA python-requests lọt cửa) — đó là lý do pipeline chạy ngon trong khi tầng
diễn giải của app này chết lặng lẽ.
...
def test_client_openai_mang_user_agent_rieng(bat_kwargs):
    ...
    assert ua, ("client openai dựng KHÔNG đặt User-Agent → SDK tự gửi "
                ...
                "exhausted' (khóa vẫn tốt) → tầng diễn giải chết lặng lẽ")
