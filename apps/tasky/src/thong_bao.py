...
def cua_leader(ma: str, user: dict, ds_nguoi: list[dict]) -> list[dict]:
    ...
    if cho_xn:
        ra.append(_muc(LUU_Y, f"{len(cho_xn)} việc chờ bạn xác nhận", len(cho_xn), "/giao-viec"))
    ...
    ket = [v for v in viec if v["so_lan_doi"] >= t.DOI_LA_KET
           and v["trang_thai"] in (t.CHO_NHAN, t.DANG_LAM, t.BAO_XONG)]
    if ket:
        ra.append(_muc(CAP, f"{len(ket)} việc của bộ phận đang kẹt", len(ket), "/giao-viec"))

    if _cuoi_tuan(ma):
        so = t.doc_tuan(ma)["dong"]
        chua = [n for n in ds_nguoi if n["ten"] not in so]
        if chua:
            ra.append(_muc(LUU_Y, f"{len(chua)} người chưa đóng tuần", len(chua), "/giao-viec"))
    return ra
