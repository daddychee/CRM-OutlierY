  ...
  return html`<div style="margin:8px 0 4px">
    <svg viewBox=${`0 0 ${w} ${h}`} style="width:100%;height:360px">
      ...
      <text x=${(w + L) / 2} y=${h - 6} font-size="11" fill="currentColor" opacity="0.75" text-anchor="middle">
        TRỤC NGANG — MỨC CẠNH TRANH${nhanCuaSo ? ` (${nhanCuaSo.toUpperCase()})` : ''}: phân vị số-video SO VỚI CÁC CỤM CÙNG LOẠI → càng phải càng đông người làm</text>
      ...
      ${d.map(r => { const len = r.phan_tram > 15, xuong = r.phan_tram < -15;
        ...
        return html`<g style="cursor:pointer" onClick=${() => onChon && onChon(r.cum)}>
          ...
          <title>${r.cum} · ${r.loai === 'mau_cau' ? 'mẫu câu' : 'đối tượng'} · ${r.video_cua_so ?? '?'} video trong cửa sổ (${r.tong_video} trọn đời) · ${r.phan_tram > 0 ? '+' : ''}${r.phan_tram}% (${r.video_30n_truoc}→${r.video_30n})${r.view_moi_ngay ? ` · ${r.view_moi_ngay} view/ngày` : ''}</title>
        </g>`; })}
...
function NutViSao({ v, onBam }) {
  // "Vì sao nóng" là nhãn HỨA QUÁ: GDELT chỉ đưa tin tức quanh mốc nóng, không
  // giải thích gì (Owner chỉ ra 23/08). Gọi đúng tên là "Chi tiết".
  if (v && v.xep_hang) return html`<button class="btn small dangchay" onClick=${onBam}>
    <i class="tr-nhay"></i>Chi tiết<${DemGiay} tu=${v.tu}/></button>`;
  if (v && v.co_du_lieu === false) return html`<button class="btn small canhbao" onClick=${onBam}
    title=${v.ly_do || ''}>Chi tiết</button>`;
  return html`<button class=${'btn small' + (v && v.co_du_lieu ? ' primary' : '')}
    onClick=${onBam}>Chi tiết</button>`;
}
...
// Popup CHI TIẾT của một thực thể — mọi thứ biết về nó ở CÙNG một chỗ, dính với
// đúng từ khoá (Owner 23/08). Ba tầng, nặng ký trước:
//   1. số đo của pool  2. VIDEO thật (có link)  3. tin tức ngoài (GDELT)
// Tầng 1-2 có sẵn nên popup mở tức thì; tầng 3 tự tải bên trong.
function ChiTietModal({ u, v, nguong, onDong, onThuLai }) {
  useEffect(() => {
    const esc = e => { if (e.key === 'Escape') onDong(); };
    window.addEventListener('keydown', esc);
    return () => window.removeEventListener('keydown', esc);
  }, [onDong]);
  const [ten, vitri, cls, nghia] = O_NHAN[u.o] || ['—', '', '', ''];
  return html`
    <div class="modalback" onClick=${e => { if (e.target === e.currentTarget) onDong(); }}>
      <div class=${'modal tr-ct ' + cls} style="max-width:720px">
        <button class="close" onClick=${onDong}>×</button>
        <div class="tr-h2" style="padding-right:26px">${u.cum}
          ${u.mo ? html`<span class="tr-mo">đang mở</span>` : null}</div>
        <div class="tr-ct-o"><i class="tr-dot"></i>${ten}<span class="tr-phu">${vitri}</span></div>
        ${u.mo_ta ? html`<p class="mut" style="font-size:12px;margin:6px 0 0">${u.mo_ta}</p>` : null}
        ${u.do_chac < 0.7 ? html`<p class="tr-ngo-to">Cụm trend gốc là “${u.khop_voi}” — thực thể này
          chỉ chiếm ${Math.round(u.do_chac * 100)}% cụm, có thể nối nhầm. Xem video bên dưới để tự kiểm.</p>` : null}

        <div class="tr-ct-so">
          <div><b>${u.n}</b><span>video của pool</span></div>
          <div><b>${u.boi}×</b><span>so trung vị pool${nguong.video_tv ? ` (ngưỡng ${nguong.boi_p75}×)` : ''}</span></div>
          <div><b>${Math.round(u.vpd)}</b><span>view/ngày (trung vị)</span></div>
          <div><b>${u.moi_nhat_ngay}n</b><span>video gần nhất</span></div>
          ${u.luong ? html`<div><b>${u.luong}</b><span>lượt tìm trên Google</span></div>` : null}
        </div>

        <div class="klabel">Video của pool nói về “${u.cum}”${
          (u.video || []).length < u.n ? ` — ${(u.video || []).length} chạy nhất trong ${u.n}` : ''}</div>
        ${(u.video || []).length
          ? (u.video || []).map(x => html`<div class="hvid">
              <a href=${'https://youtu.be/' + x.yt_id} target="_blank" rel="noopener">
                <img loading="lazy" src=${'https://i.ytimg.com/vi/' + x.yt_id + '/mqdefault.jpg'} alt=""/></a>
              <div><div class="hvt"><a href=${'https://youtu.be/' + x.yt_id} target="_blank" rel="noopener">${x.title}</a></div>
              <div class="hvm"><b>${soGon(x.views)} view</b> · ${soGon(x.vpd)}/ngày${
                x.tuoi != null ? ' · ' + x.tuoi + ' ngày tuổi' : ''}${x.kenh ? ' · ' + x.kenh : ''}</div></div>
            </div>`)
          : html`<p class="mut" style="font-size:12.5px">Pool có ${u.n} video khớp nhưng bản ghi thiếu mã
              video nên không dựng được link — không bịa link rỗng.</p>`}

        <div class="klabel">Tin tức quanh mốc nóng — GDELT</div>
        <p class="mut" style="font-size:12px;margin:0 0 6px">Đây là <b>bằng chứng để tự đọc</b>, không phải
          lời giải thích: GDELT chỉ cho biết lượng tin nhắc tới “${u.cum}” theo ngày và những bài tạo ra đỉnh.</p>
        ${!v ? html`<p class="mut">Đang xin…</p>`
          : v.xep_hang ? html`<p class="mut"><i class="tr-nhay"></i>
              ${v.vi_tri === 0 ? html`Đang hỏi GDELT<${DemGiay} tu=${v.tu}/> — nguồn này thường mất 12–17 giây.`
          ...
          : !v.co_du_lieu ? html`<p class="mut">${v.ly_do}
              <button class="btn small" style="margin-left:8px" onClick=${onThuLai}>Thử lại</button></p>`
          : html`<div>
              <p class="mut" style="font-size:12px;margin:0">Đỉnh <b>${v.dinh_ngay}</b> ·
                ${v.dinh_phan_tram} ${v.don_vi}${v.tu_cache ? ' · đã lưu' : ''}</p>
...
function Trending({ ws, canEdit }) {
  ...
  // Bấm là MỞ NGAY. Popup dựng được liền từ dữ liệu pool (số đo + video); riêng
  // khối GDELT tự tải bên trong và tự hiện trạng thái của nó. Trước phải chờ
  // GDELT xong mới thấy gì — trong khi phần nặng ký nhất (video của pool) đã sẵn.
  const hoiViSao = cum => {
    const cu = viSao[cum];
    setChon(cum);
    if (!cu || cu.co_du_lieu === false) xinViSao(cum, !!(cu && cu.co_du_lieu === false));
  };
  ...
  return html`<div class="tr-wrap">
    ...
    ${!kq ? html`<div class="card mut">Chưa có lượt quét nào.${d.duoc_quet ? ' Bấm Quét để bắt đầu.' : ''}</div>`
      ...
      ${(() => {
        const u = (kq.ung_vien || []).find(x => x.cum === chon);
        return u ? html`<${ChiTietModal} u=${u} v=${viSao[chon]} nguong=${kq.nguong || {}}
          onThuLai=${() => xinViSao(chon, true)} onDong=${() => setChon(null)}/>` : null;
      })()}
...
function Mapping({ ws, canEdit }) {
  ...
  const [moHot, setMoHot] = useState(null);              // dòng đang mở trong khối Topic/Hook hot
  const [busy, setBusy] = useState('');
  const [err, setErr] = useState('');
  ...
  return html`
    ...
    <div class="panel" style="border-left:4px solid var(--accent,#4C8FE0)">
      ...
      ${noi && noi.hot && ((noi.hot.topic || []).length || (noi.hot.hook || []).length)
        ? html`<div class="hotwrap">
          ${[['topic', 'Topic hot', 'chủ thể đang nóng', 'doi_tuong'],
             ['hook', 'Hook hot', 'cách đặt tiêu đề đang ăn', 'mau_cau']].map(([k, ten, ghi, tab]) =>
            (noi.hot[k] || []).length ? html`<div class="hotcol">
            <div class="hothead"><b>${ten}</b><span class="note" style="margin:0">${ghi}</span></div>
            ${(noi.hot[k] || []).map((it, i) => { const id = k + '|' + it.cum;
              const nenH = noi.nong_meta && noi.nong_meta.nen; return html`
            <div class=${'hrow' + (moHot === id ? ' open' : '')}>
              <div class="hmain" onClick=${() => setMoHot(moHot === id ? null : id)}>
                <span class="hrank">${i + 1}</span>
                <span><span class="hname">${it.cum}</span><div class="hsub">${it.so_no}/${it.so_moi} video mới nổ${
                  (it.hook || []).length ? html` · hook đang ăn: <span class="tag">${it.hook.join(' · ')}</span>` : ''}</div></span>
                <span class="hno"><b>${Math.round((it.ti_le_no || 0) * 100)}%</b><small>${nenH ? '×' + (it.ti_le_no / nenH).toFixed(1) + ' nền' : 'tỉ lệ nổ'}</small></span>
              </div>
              ${moHot === id ? html`<div class="hdrawer">
                <div class="klabel">Video nổ (${(it.video_no || []).length})</div>
                ${(it.video_no || []).map(v => html`<div class="hvid">
                  <a href=${'https://youtu.be/' + v.yt_id} target="_blank" rel="noopener">
                    <img loading="lazy" src=${'https://i.ytimg.com/vi/' + v.yt_id + '/mqdefault.jpg'} alt=""/></a>
                  <div><div class="hvt"><a href=${'https://youtu.be/' + v.yt_id} target="_blank" rel="noopener">${v.title}</a></div>
                  <div class="hvm"><b>${soGon(v.views)} view</b> · ${soGon(v.vpd)}/ngày${v.tuoi ? ' · ' + v.tuoi + ' ngày tuổi' : ''} · ${v.kenh}</div></div>
                </div>`)}
                <div class="klabel">Kênh đang đẩy</div>
                <div class="kchips">${(it.kenh_day || []).map(kk => html`<span class="kchip"><b>${kk.ten}</b> · ${kk.so} video · ${soGon(kk.view)} view${kk.no ? html` · <span class="no">${kk.no} nổ</span>` : ''}</span>`)}</div>
                <div class="row" style="gap:8px;margin-top:11px">
                  <button class="btn small primary" onClick=${() => traCuu(it.cum)}>Tra cứu từ khóa này</button>
                  <button class="btn small ghost" onClick=${() => setLoaiCum(tab)}>Mở tab ${k === 'topic' ? 'Topic' : 'Hook'}</button>
                </div>
              </div>` : ''}
            </div>`; })}
          </div>` : '')}
        </div>
        <div class="note" style="margin-top:9px">Đang nóng · cửa sổ ${(noi.nong_meta && noi.nong_meta.cua_so_ngay) || 60} ngày · nền nổ chung của pool ${noi.nong_meta && noi.nong_meta.nen ? Math.round(noi.nong_meta.nen * 100) + '%' : '—'} — bấm một dòng để xem video và kênh đứng sau con số.</div>` : ''}
    </div>
    ...
    <div class="panel">
      ...
      ${(noi && (noi.cum || []).length) ? html`<div style="margin-top:10px">
        <div class="note" style="margin:0 0 3px">Từ khoá trong pool — đang lên / đang giảm
          ...
          ${!noi.tu_de ? html`<span> · lọc ngôn ngữ:
            <select value=${noi.ngon_ngu_loc || ''} onChange=${e => {
              ...
              api('GET', `/workspaces/${ws}/discovery/tu-khoa-noi?cua_so=${cuaSo}&ngon_ngu=${encodeURIComponent(e.target.value)}`)
                .then(setNoi).catch(() => {});
            }}>
        ...
        <div class="row" style="gap:8px;align-items:center;flex-wrap:wrap;margin-top:8px">
          <span class="note" style="margin:0">Cửa sổ đo:</span>
          <select style="background:var(--panel);color:var(--text);border:1px solid var(--line);border-radius:8px;padding:4px 8px;font:inherit;font-size:13px;cursor:pointer"
            value=${cuaSo}
            onChange=${e => {
              const cs = Number(e.target.value);
              setCuaSo(cs);
              const nn = noi.ngon_ngu_loc || '';
              api('GET', `/workspaces/${ws}/discovery/tu-khoa-noi?cua_so=${cs}`
